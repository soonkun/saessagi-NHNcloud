...
> ## ⚠️ CR-38 (2026-07-29)로 이 문서의 대부분은 무효다
>
> Electron 앱이 제거되고 UI가 브라우저 웹 페이지가 되면서, 아래 제약의 근거였던
> **투명창·클릭 관통(click-through)·항상 위·펫 모드가 전부 사라졌다.**
> 관련 코드(`CharacterWidget.tsx`, `clickthrough.ts`, `frontend/`)도 삭제됐다.
>
> **여전히 유효한 것**: TTS 재생 관련 주의사항, 그리고 웹 UI의 새 제약 —
> 마이크(`getUserMedia`)는 HTTPS(secure context)에서만 동작한다.
>
> 역사적 맥락 참고용으로 남겨둔다. 새 기능 개발 시 이 문서의 투명창 관련 내용을
> 근거로 삼지 말 것.

이 문서는 (당시) Electron 투명창 특성상 절대 위반해서는 안 되는 제약을 정의한다.
