...
### E-102: 그래프가 문서를 잘 고르고 **표지를 근거로 넘김** (CR-61, 2026-08-09)

**발견 경위**: 사용자가 "단순 임베딩 검색과 엔티티 기반 검색이 어떤 차이가 있는지
검토해줘"라고 물어 두 경로를 비교하다 나왔다. 버그 신고가 아니라 **설계 검토에서**
드러난 결함이다.

**단서**: 질의 6건 전부 두 경로의 결과가 **한 청크도 겹치지 않았다.**
```
벼 병해충 방제 기술      벡터10 + 그래프10 → 합집합 20 · 겹침 0
스마트팜 자동화 온실 제어  …                          겹침 0   (6/6 동일)
```
상호보완이라 좋은 신호로 볼 수도 있지만, 같은 문서를 골랐다면 청크가 일부라도 겹쳐야
자연스럽다. 0이라는 것은 **청크를 고르는 방식이 서로 무관하다**는 뜻이었다.

**원인**: `kg/retrieve.py`가 문서를 랭킹한 뒤
```python
chunk_rows = vstore.get_chunks_by_doc_id(doc_id, limit=2)
```
`get_chunks_by_doc_id`는 `search().where(doc_id=…).limit(2)` — **정렬이 없다.**
사실상 문서의 앞 2청크, 즉 표지와 제출문이다. 실제 확인:
```
[1] p1: 완결과제 최종보고서 … (과제번호 : PJ012614) …
[2] p2: 제 출  농촌진흥청장 귀하 … "놓업분야 용심가스 강축 기슐" (OCR 깨짐)
```
질문이 무엇이든 그 문서에서는 **항상 같은 두 청크**가 갔다.

엔티티 검색의 강점은 df 역가중으로 **어느 문서인지**를 정확히 맞히는 것인데, 그 성과를
마지막 한 줄에서 버리고 있었다. 좋은 문서를 고른 뒤 쓸모없는 부분을 인용한 셈이다.

**영향(실측)**: 최종 근거 50청크 중 **14건(28%)이 문서 1~2페이지**.
근거의 1/4 이상이 표지·제출문이었고, 딥 리서치도 같은 `hybrid_retrieve`를 쓰므로
동일하게 영향받았다(근거 24건 규모라 절대량은 더 컸다).

**수정**:
- `VectorStore.search_in_doc(query_vec, doc_id, top_k)` 신설 — 문서 **안에서**
  질의 벡터로 다시 검색한다.
- `KgRetriever.retrieve(..., query_vec=)` — 호출자가 질의 임베딩을 **한 번만** 만들어
  넘기고 문서마다 재사용한다. 문서 수만큼 임베딩이 늘지 않는다.
- 임베딩 실패·`search_in_doc` 없는 저장소·문서 내 결과 0건이면 **예전 경로로 폴백**.
  로그에 `청크선택=문서내검색 / 앞청크(폴백)`을 남겨 어느 쪽이 돌았는지 보이게 했다.
- 두 경로가 **같은 row 형태**를 내도록 `_hit_to_row()`를 뒀다 — 호출자가 분기하지 않는다.

**검증**: 같은 질의 5건 재측정 — **28% → 12%**.
로그 5건 전부 `청크선택=문서내검색`(폴백 0회), 문서 다양성은 6~8건으로 유지.
남은 12%는 벡터 경로가 스스로 앞쪽을 고른 것으로, 요약·초록이 앞에 있는 문서에서는
정상이다. 회귀 테스트 4건(문서내 검색 사용·벡터 없을 때 폴백·옛 저장소·형태 일치).

**교훈**:
- **파이프라인은 가장 약한 마지막 단계만큼만 좋다.** 엔티티 매칭·IDF 랭킹·상용구 감점을
  정교하게 만들어 놓고 마지막 `limit=2` 한 줄이 전부를 무효화했다. 앞단 품질에 공을
  들일 때 **결과가 실제로 어떤 텍스트로 나가는지** 끝까지 따라가 볼 것.
- **"검색이 문서를 찾았다"와 "LLM이 쓸 만한 근거를 받았다"는 다른 문제다.** 문서 단위
  지표(정확도·재현율)만 보면 이 결함은 보이지 않는다. 실제로 주입되는 텍스트를 눈으로
  읽어야 발견된다.
- **겹침 0%는 축하할 일이 아니라 의심할 일이었다.** 두 검색이 같은 코퍼스를 보는데
  결과가 완전히 분리되면, 상호보완이 아니라 **한쪽이 엉뚱한 것을 보고 있을** 가능성을
  먼저 확인해야 한다.
- 이 결함은 사용자 신고가 아니라 **비교 검토 요청**에서 나왔다. "왜 이렇게 동작하나"를
  설명하려고 코드를 따라가는 것이 버그를 찾는 유효한 방법이다.

---

### E-103: 재진입하면 근거 링크가 사라지고, 접두사 없는 마커가 날것으로 노출 (CR-64, 2026-08-09)

**증상**: 사용자 지적 — "대화방을 나갔다가 다시 들어오면 근거문서 링크가 자꾸 사라져."
그리고 본문에 `[[TRKO202500017741_농업분야기후변화….pdf_afbc4f63]]`가 그대로 보였다.

**원인 둘.**

**(1) 칩이 런타임 상태였다.** `citedDocs`는 실시간 턴에만 메시지에 붙었고
`history-data` 복원은 `{id, role, text, timestamp}`만 되살렸다. 저장된 원문에는
`[[doc:…]]` 마커가 멀쩡히 남아 있었는데 그걸로 칩을 재구성하지 않았다.

**(2) 제거 정규식이 접두사를 요구했다.** 백엔드·프론트 둘 다
`\[\[(?:doc|note):` 였는데, 실측 저장 이력의 이중괄호 **16건 중 11건**이
`doc:` 없는 `[[<doc_id>]]` 형태였다. 하나도 안 걸러져 화면에 날것으로 떴다.
`신기후변화대응체계구축사업최종·종보고서.pdf`처럼 파일명이 깨진 것도 있었다.

**수정**: 마커를 **본문에 남기는 방식**으로 바꿨다 — 텍스트에 있으니 히스토리에서
복원해도 저절로 살아난다. 별도 상태가 필요 없다. 접두사 누락은 지우지 않고
`[[doc:…]]`로 되살리고, 깨진 이름은 정규화 비교로 후보가 하나면 교정한다.
검색 결과에 없는 문서(환각)만 제거한다.

**교훈**: **표시에 필요한 정보는 표시되는 데이터 안에 있어야 한다.** 칩을 별도
런타임 상태로 들고 있으면 그 상태를 만드는 경로(실시간 스트리밍)에서만 동작하고,
다른 경로(히스토리 복원)에서는 조용히 사라진다. 마커를 본문에 남기자 두 경로가
저절로 같아졌다 — 상태를 없애는 것이 상태를 동기화하는 것보다 낫다.

---

### E-104: 인용 링크가 `[근거](saessagi-doc:12.%20(%EA%B3%A0…` 암호문으로 노출 (CR-64, 2026-08-09)

**증상**: 사용자 지적 — "마지막에 저 암호문은 대체 뭐야?" 본문 끝에 퍼센트 인코딩된
문자열이 수십 줄 쏟아졌다.

**원인**: 인용 칩을 마크다운 링크(`[근거](saessagi-doc:<doc_id>)`)로 만들었는데
**doc_id에 괄호가 든 문서**가 있었다: `12. (고정) 드론영상 기반 관수.pdf_abc123`.
마크다운 링크 URL에 괄호가 있으면 파서가 링크를 만들지 못하고 원문을 그대로 뱉는다.
`encodeURIComponent`로는 못 막는다 — **`(`와 `)`를 인코딩하지 않는다.**

**수정**: URL에 **번호만** 넣는다 (`saessagi-doc:0`). 숫자뿐이라 어떤 파일명이 와도
깨지지 않는다. 칩 렌더러가 그 번호로 본문의 doc_id 목록을 조회한다.

**함께 고친 것**:
- react-markdown의 기본 URL 정제기가 커스텀 스킴을 **지워** href가 비었다.
  백엔드가 인라인 8개를 보냈는데 화면 칩이 0개였다 → `urlTransform`으로 우리 스킴만 통과.
- 마커가 **표 안**에 들어가 셀에 칩이 박히고 모델이 '관련 근거' 열까지 만들었다
  → 표 줄에서 마커를 빼고 표가 끝난 뒤에 낸다. 프롬프트에도 금지 문구 추가.
- 칩이 문장마다 흩어졌다 → 상위 항목(`#`, `1.`) 단위로 묶는다. 하위 불릿·빈 줄은
  경계로 보지 않는다.

**교훈**:
- **식별자를 URL·마크다운 같은 문법 안에 직접 끼워 넣지 말 것.** 파일명에는 괄호·공백·
  점이 들어간다. 인덱스처럼 **문법에 안전한 값**으로 한 겹 감싸면 데이터가 문법을
  깨뜨릴 수 없다. 이스케이프로 막으려던 첫 시도는 실패했다(`encodeURIComponent`가
  괄호를 남긴다).
- **데이터 의존 버그는 내 테스트 질의에서 안 나온다.** 괄호가 든 파일명을 우연히
  안 건드려서 나는 재현하지 못했고, 사용자 화면에서만 터졌다. 실데이터의 **극단값**
  (가장 긴 이름, 특수문자 든 이름)으로 시험할 것.

---

### E-105: 인용 칩을 메시지 단위로 중복 제거해 근거가 확 줄어듦 (CR-64, 2026-08-09)

**증상**: 사용자 지적 — "근거를 하나만 들 필요가 없잖아? 관련된 근거를 참고했으면
다 달아야지. 근거칩이 확 줄어든 느낌이야." 섹션마다 칩이 1개씩만 남았다.

**원인**: 앞선 요청("같은 자료를 반복해서 달지 마세요")을 **메시지 전체 단위**로
구현했다. 한 자료가 3개 섹션의 근거여도 첫 섹션에만 남고 나머지는 제거됐다.
사용자의 원래 뜻은 **한 문장·한 자리에 반복되는 것**을 막자는 것이었다.

또 모델이 검색된 자료 일부에만 마커를 달았는데(실측 8건 중 3건) 나머지를 버려서
그 자료에 접근할 방법이 없었다.

**수정**:
- 백엔드는 중복을 지우지 않는다. 같은 섹션 안의 중복 정리는 프론트가
  `groupMarkersByBlock`에서 맡는다 — 섹션 경계를 아는 쪽이 거기다.
- 모델이 빠뜨린 자료는 **본문 뒤에 보탠다.** 검색된 자료는 전부 컨텍스트에
  들어갔으므로 실제로 참고된 것이다. 본문 인용은 제자리에, 남은 것만 뒤에.

**검증**: 같은 질의에서 칩 3개 → **8개**(본문 8 + 미인용 2, 섹션 반복 포함).

**교훈**: **"중복을 지워라"는 범위를 정해야 실행 가능한 지시가 된다.** 같은 말이
문장 단위·섹션 단위·문서 단위에서 전혀 다른 결과를 낸다. 범위를 안 물어보고 가장
넓게 잡았다가 사용자가 원한 정보를 지웠다. 정리·필터링 요구는 **어느 단위인지**를
확인할 것.

---

### E-107: 지식그래프에서 검색만 하면 화이트스크린 (CR-61 회귀, 2026-08-10)

**증상**: 사용자 지적 — "지식그래프에서 검색만 해도 그냥 화이트스크린 떠 버리는데."
검색어를 2자 이상 입력하는 순간 화면이 하얘졌다.

**재현**: 헤드리스로 검색어를 넣자 `본문 0자 · root 자식 0개`가 되고
```
TypeError: Cannot read properties of undefined (reading 'length')
```

**원인**: **응답 형태와 프론트 타입이 어긋났다.** CR-61에서 과제 검색을 M_19 키워드에서
M_23 엔티티로 교체할 때 백엔드 응답 필드가 바뀌었는데 프론트를 안 고쳤다.
```
옛 응답 : doc_id, title, project_no, title_match, matched_keywords
새 응답 : doc_id, title, doc_name, year, document_type, score
```
프론트는 `{d.matched_keywords.length > 0 && …}`를 계속 읽었다. 없는 필드라
`undefined.length`에서 예외가 나고, React가 그 아래 트리를 통째로 버려 흰 화면이 됐다.

**왜 컴파일러가 못 잡았나** — 이게 이 사고의 핵심이다.
- 프론트 `GraphDocMatch`에 `matched_keywords: string[]`이 **선언돼 있었다.** tsc 입장에서는
  완벽히 정상인 코드다. 타입 선언이 실제 응답과 어긋나면 타입 검사는 아무것도 못 한다.
- 백엔드는 `DocSearchResp(docs: list[dict[str, Any]])`로 반환해 **형태 검증이 없었다.**
  pydantic이 `dict[str, Any]`를 그냥 통과시킨다.
- 즉 계약이 **양쪽 어디에도 강제되지 않았다.** 서버는 아무 dict나 보낼 수 있고, 프론트는
  아무 필드나 있다고 선언할 수 있다.

**수정**:
- 프론트 타입을 실제 응답 필드로 맞췄다. `score>=2`면 제목 일치(옛 `title_match` 대체),
  사라진 `matched_keywords` 자리에는 연도·파일명을 보여 준다.
- `searchGraphDocs`가 배열이 아닌 응답을 받으면 **빈 목록으로 떨어진다.** 형태가 또
  바뀌어도 화면이 하얘지지 않는다.
- 회귀 테스트 3건 — M_23 응답 파싱, `docs` 없음, `docs`가 배열 아님.

**교훈**:
- **타입 선언은 계약이 아니라 주장이다.** 서버 응답과 맞는지 아무도 확인해 주지 않는다.
  경계에서 실제로 검증하거나(파싱), 최소한 없을 수 있는 필드는 옵셔널로 선언할 것.
- **`list[dict[str, Any]]`를 반환하는 엔드포인트는 프론트와의 계약을 포기한 것이다.**
  이 저장소에는 그런 라우트가 여럿 있다(`/search-docs`, `/api/kg/*`). 필드를 바꿀 때
  프론트를 같이 고쳤는지 확인할 방법이 없다.
- **렌더 중 예외 하나가 화면 전체를 날린다.** React는 그 트리를 버린다. 목록 항목을
  그릴 때 서버에서 온 값의 `.length`·`.map`을 직접 부르는 것은 위험하다.
- E-104(대괄호 파일명)·E-103(접두사 없는 마커)과 같은 성격이다 — **데이터가 코드의
  가정을 벗어날 때** 터진다. 세 번 반복됐으니 경계에서 방어하는 습관이 필요하다.
