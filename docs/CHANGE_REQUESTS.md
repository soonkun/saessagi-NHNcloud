...
3. **햄버거를 한 줄 아래 전용 메뉴 줄로 이동**(사용자 제안: "한칸 내려와야 맞을 것 같고").
   맨 윗줄이 또 가려지더라도 메뉴는 살아남는다 — 같은 사고가 재발해도 갇히지 않게 하는
   안전장치다. 터치 대상 크기도 44×40으로 키우고, 지금 보고 있는 탭 이름을 함께 보여준다.

**재검증**(402×774, iPhone UA): body 높이 = 뷰포트 일치 / `window.scrollY === 0`(밀리지 않음) /
자동 포커스된 요소 없음(`BODY`) / 햄버거 y=40·44×40·클릭 가능·눌러서 메뉴 열림 /
새 대화 버튼 y=94·클릭 가능 / 콘솔 에러 0.
데스크탑(1440)은 메뉴 줄 없이 사이드바 그대로, 자동 포커스 유지. 태블릿(820)은 메뉴 줄 표시.
