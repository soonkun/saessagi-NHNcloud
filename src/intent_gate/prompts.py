...
_FEW_SHOT: list[tuple[str, dict[str, object]]] = [
    ...
    # ── followup (CR-51) ───────────────────────────────────────────────────────
    # 핵심은 "정리/요약" 같은 단어가 아니라 **문장 안에 주제가 있는가**다.
    # 예전 정규식은 단어만 보고 새 질문까지 후속으로 잡았다 (E-79).
    (
        "짧게 정리해줘",
        {
            "intent": "followup",
...
SYSTEM_PROMPT: str = f"""당신은 사용자 발화를 7개 의도 라벨 중 하나로 분류하는 전문 분류기입니다.
반드시 JSON 형식으로만 답하세요.
...
5. **followup vs 나머지 전부**: 판단 기준은 단어가 아니라 **문장 안에 주제가 있는가**다.
   - "정리해줘" / "요약해줘" 같은 표현이 있다고 followup이 아니다.
