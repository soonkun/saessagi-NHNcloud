...
class AppServiceContext(ServiceContext):  # type: ignore[misc]
    ...
    async def construct_system_prompt(self, persona_prompt: str) -> str:
        ...
        notes_block = (
            ...
            "## RAG 인용 마커\n"
            "참고한 자료는 **마커**로 표시하세요:\n"
            "- 일반 문서: `[[doc:<doc_id>]]` — doc_id는 search_docs hit의 `doc_id` 값 또는\n"
            "  [관련 문서 검색 결과] 블록의 doc_id를 **그대로** 복사. `doc:` 접두사를 빼먹지 마세요.\n"
            "- 업무 노트: `[[note:<slug>]]`\n"
            "마커는 사용자에게 짧은 칩으로 보이며, 누르면 원문이 열립니다.\n\n"
            "**마커는 반드시 답니다. 다만 자료당 한 번입니다.**\n"
            "- 참고한 자료는 **각각 한 번씩** 마커를 답니다. 마커가 없으면 사용자가 원문에\n"
            "  접근할 수 없습니다.\n"
            "- 위치는 **큰 항목(1., 2., ## 제목)이 끝나는 자리**입니다. 문장마다 달지 마세요.\n"
            "- 같은 자료를 여러 항목에 **반복해서 달지 마세요** — 한 번이면 충분합니다.\n"
            "- **표 안에는 절대 넣지 마세요.** '관련 근거' 같은 열도 만들지 마세요.\n"
            "  표가 깨집니다. 마커는 표가 끝난 **다음 줄**에 답니다.\n"
            "- 검색 결과에 없는 문서명을 지어내지 마세요 — 그런 마커는 제거됩니다.\n"
        )
